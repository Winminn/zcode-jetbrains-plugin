...
- **取消引导假成功致消息重复发送**：AI 回复生成中点「取消引导」，消息看似已撤回、实际仍会注入当前回合（删除排队消息同理）；现在撤销以服务端确认为准，注入完成后取消会明确提示，不再出现同一句话发两条。
...
- **False-success on steer cancel caused duplicated messages**: cancelling a steer message while the AI is replying looked successful while the message still got injected into the turn (same for deleting a queued message); the cancel result now follows the server's confirmation, a clear notice is shown when injection has already happened, and the same text is no longer sent twice.
