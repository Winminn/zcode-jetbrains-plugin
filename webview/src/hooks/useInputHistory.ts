...
function loadHistory(): string[] {
  ...
  try {
    ...
    memoryHistory = Array.isArray(parsed)
      ? parsed.filter(
          (x): x is string =>
            typeof x === 'string' && x.length <= MAX_INPUT_HISTORY_TEXT_LENGTH,
        )
      : []
  } catch {
...
function loadCounts(): Record<string, number> {
  try {
    ...
    for (const [k, v] of Object.entries(parsed)) {
      if (k.length <= MAX_INPUT_HISTORY_TEXT_LENGTH && typeof v === 'number' && Number.isFinite(v))
        out[k] = v
    }
    return out
  } catch {
...
function loadTimestamps(): Record<string, string> {
  try {
    ...
    for (const [k, v] of Object.entries(parsed)) {
      if (k.length <= MAX_INPUT_HISTORY_TEXT_LENGTH && typeof v === 'string') out[k] = v
    }
    return out
  } catch {
    return {}
